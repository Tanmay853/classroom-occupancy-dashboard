...
# image_directory = 'imgs' # Directory to scan for new images
...
# # Dictionary to map image file names to their source/destination points
# # This replaces the hardcoded list and allows for dynamic lookup.
# image_point_data = {
#     'LC302_1_1.png': {
...
#     """
#     Finds the latest images in a directory.
#     Assumes image files have common extensions like .png, .jpg, .jpeg.
#     """
...
#                 if image_filename in image_point_data:
#                     src_points = image_point_data[image_filename]['src_points']
#                     dst_points = image_point_data[image_filename]['dst_points']
...
#                 else:
#                     print(f"Warning: No homography points found for {image_filename}. Skipping...")
...
#         print("\nWaiting for 0.5 minutes before the next run...")
#         time.sleep(30) # Wait for 30 seconds (0.5 minutes)
...
API_KEY = "Z2ZjbWt3Zjk2dTRrdjg2NTc1b21mOmM0a3Y4MzdiMFplTkJBM3NTMlZ4ZEtsNVRhV2dGajdC"
layout_image_path = 'incoming/lc001.png'  # Plan view image
image_directory = 'incoming' # Directory to scan for new images
num_images_to_process = 3 # Number of latest images to process each cycle
...
# Dictionary to map image file names to their source/destination points
# Note: The keys are simplified names based on the camera number
image_point_data = {
    'LC302_1.png': {
        'src_points': [[170, 269], [313, 284], [470, 273], [657, 256], [98, 688], [468, 690], [1238, 690], [1021, 284], [1234, 167], [1121, 82], [1052, 49], [424, 44], [617, 96], [1034, 51], [917, 27], [981, 41], [1035, 63], [922, 43], [1270, 254], [1206, 241], [1120, 227]],
        'dst_points': [[115, 44], [125, 96], [123, 132], [123, 179], [45, 83], [51, 138], [47, 227], [122, 240], [179, 421], [265, 417], [304, 414], [239, 56], [207, 169], [252, 324], [248, 266], [252, 293], [229, 317], [229, 268], [121, 329], [120, 300], [122, 273]]
...
def get_latest_images(directory, num_images):
    ...
    """
    files = [os.path.join(directory, f) for f in os.listdir(directory) if f.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp'))]
    files.sort(key=os.path.getmtime, reverse=True)
    return files[:num_images]
    ...
    while True:
        ...
        if not latest_image_paths:
            ...
            for image_path in latest_image_paths:
                ...
                try:
                    # Example: 'LC302_L1_2F_CAM_1 (62)_...' -> '1'
                    cam_number = image_filename.split('_')[4]
                    if ' ' in cam_number:
                        cam_number = cam_number.split(' ')[0]
                    # Create a simplified key to look up homography data
        ...
        print("\nWaiting for 5 minutes before the next run...")
