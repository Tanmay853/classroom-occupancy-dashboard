...
# # 5️⃣ Load the Trained Model (Next Time)
# trained_model = YOLO(best_model_path)

# # 6️⃣ Run Inference to Test the Trained Model
# results = trained_model("C:/Users/DELL/Downloads/homography-computation-master/homography-computation-master/imgs/class.jpg")
# # Iterate over results
# for result in results:
#     result.show()  # Show detected objects correctly
...
src = cv.imread('imgs/kabaddi.jpg', -1)
src_copy = src.copy()
cv.namedWindow('src')
cv.moveWindow("src", 80,80)
cv.setMouseCallback('src', select_points_src)

dst = cv.imread('imgs/kabaddi_layout.jpg', -1)
dst_copy = dst.copy()
cv.namedWindow('dst')
